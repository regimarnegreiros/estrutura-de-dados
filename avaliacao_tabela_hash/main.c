...
NoChaveValor *obterValor(HashTable *ht, int chave) {
    if (!ht) return NULL;
...
void *deletarChave(HashTable *ht, int chave) {
    if (!ht) return NULL;
...
int main() {
    ...
    NoChaveValor *v = obterValor(ht, 29);
    printf("Encontrado -> Chave: %d, Nome: %s, Idade: %d\n\n", v->chave, v->valor.nome, v->valor.idade);
